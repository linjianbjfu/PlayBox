...
public:
	...
	std::string get_file_description() const { return get_value("FileDescrition"); }

	DWORD get_product_version_ms()
	{
		if(resource_data_)
		{
			UINT              size   = 0;
			...
			if(::VerQueryValue(static_cast<LPVOID>(resource_data_),
				...
				(LPVOID*)(&version_info),
				&size) != FALSE)
				return version_info->dwProductVersionMS;
		}
