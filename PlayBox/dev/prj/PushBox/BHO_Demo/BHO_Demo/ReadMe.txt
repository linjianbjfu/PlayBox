...
应用程序向导已为您创建了此 BHO_Demo 项目，作为编写动态链接库 (DLL) 的起点。
...
BHO_Demo.idl
    ...
    此文件将由 MIDL 编译器进行处理以生成：
        C++ 接口定义和 GUID 声明 (BHO_Demo.h)
        GUID 定义                                (BHO_Demo_i.c)
        类型库                                  (BHO_Demo.tlb)
        封送处理代码                                 （BHO_Demo_p.c 和 dlldata.c）

BHO_Demo.h
    此文件包含 BHO_Demo.idl 中定义的项目的 C++ 接口定义和 GUID 声明。它将在编译过程中由 MIDL 重新生成。
...
StdAfx.h, StdAfx.cpp
    这些文件用于生成名为 BHO_Demo.pch 的预编译头 (PCH) 文件和名为 StdAfx.obj 的预编译类型文件。
...
BHO_Demops.vcproj
	...
	dlldata.c、BHO_Demo_i.c 和 BHO_Demo_p.c，这些文件是
