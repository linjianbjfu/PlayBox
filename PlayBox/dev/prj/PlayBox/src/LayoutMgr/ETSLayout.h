...
namespace ETSLayout
{
	...
	typedef	CFrameWnd			CBaseFrameWnd;
 ...
 */
const int nDefaultBorder	= 0;//5;

/**
 * The minimum size for not ABSOLUTE_XXX items
 */
const int nMinConstrain = 0;//5;
...
public:
	...
	enum layPositionMode {
		NONE				= 0,
		START_X_FROM_LEFT	= 1,
		START_X_FROM_RIGHT	= 1 << 1,
		START_Y_FROM_TOP	= 1 << 2,
		START_Y_FROM_BOTTOM = 1 << 3,
		END_X_FROM_LEFT		= 1 << 4,
		END_X_FROM_RIGHT	= 1 << 5,
		END_Y_FROM_TOP		= 1 << 6,
		END_Y_FROM_BOTTOM	= 1 << 7,
		START_X_PERCENT		= 1 << 8,
		START_Y_PERCENT		= 1 << 9,
		END_X_PERCENT		= 1 << 10,
		END_Y_PERCENT		= 1 << 11,
		...
		DRAW_NO_STRECTH		= 1 << 15,
		DRAW_TILE			= 1 << 16,

		DRAW_TRANSPRANT		= 1 << 17,
		DRAW_CENTER_STRECH	= 1 << 18
...
*/
	...
	CMapStringToString   m_mapParams;

	CString	m_strWndName;

	CString m_strControlName;

	void	SetName(CString wndName, CString controlName){m_strWndName = wndName; m_strControlName = controlName;};
	/**
	 *	可以添加参数给start_x_left等用
	 */ 
	...
	public:
		 ...
		 */
		virtual bool	resizeTo(CRect& rcNewArea, bool bRedraw = false, bool bReSizeAlways = false) = 0;

		virtual CPaneBase GetSubPane(CString name){return CPaneBase(NULL);};
...
//		virtual	void adjustSize() = 0;
		...
		CRect getPaneRect(){return m_rcRect;};

		CArray<float, float> m_aryGreedyProp;
		...
		void	setPosition(CString start_x, CString start_y , CString end_x, CString end_y, CString percent_start, CString percent_end, layPositionMode mode);
	protected:
	...
	public:
		...
		CString m_percent_start;

		CString m_percent_end;

		layPositionMode	 m_posMode;

		COLORREF	 m_clrTrans;

		PaneBase*	 m_paneParent;
	...
	class PaneMulti;
	class ETSGUI_EXT_CLASS CPaneMulti : public CPane
	{
	public:
		// Standardconstructor
		CPaneMulti( );
		CPaneMulti( PaneMulti* pPane );
	//	CPane( const CPane& other );
	//	~CPane();
		CPaneMulti& operator<< ( const CPane pPane );
		CPaneMulti& operator<< ( const CPaneBase pItem );

		PaneMulti* operator->() const;
	};

	class PanePosition;
	class ETSGUI_EXT_CLASS CPanePosition : public CPane
	{
		friend class PaneBase;
	public:
		// Standardconstructor
		CPanePosition( );
		CPanePosition( PanePosition* pPane );

		CPanePosition& operator<< ( const CPane pPane );
		CPanePosition& operator<< ( const CPaneBase pItem );

		PanePosition* operator->() const;
	};
	...
	public:
		...
		virtual bool resizeTo(CRect& rcNewArea,bool bRedraw = false, bool bReSizeAlways = false);
		virtual void showPane(bool bShow);
		bool	isDummy()				{ return (m_hwndCtrl == 0);	}
		void	setItemType(ETSLayoutMgr::itemType type){m_itemType = type;};
	protected:
	...
	class ETSGUI_EXT_CLASS Pane : public PaneBase {
		...
		friend class CPaneBase;
		friend class CPane;
		friend class CPaneMulti;
		friend class CPanePosition;
        friend class ETSLayoutPropertySheet;
	...
	public:
		 ...
		 */
		virtual bool addPane( CPaneBase pItem );
		virtual bool addPane( CPane pSubpane, layResizeMode modeResize, int sizeSecondary /* = 0 */);
		...
		CPaneBase GetSubPane(CString name);
	...
	CPane paneSpl(ETSSplitter * pSpl,  layOrientation orientation, ETSLayoutMgr::layResizeMode modeResize =GREEDY, 
									   int nPos = 0, int sizeExtraBorder =0, 
									   int sizeSecondary =0,
									   int isClosed=false,
									   int nHideCloseBar=false,
									   int nSlim=false
				);

	CPaneMulti paneMulti( layOrientation orientation, ETSLayoutMgr::layResizeMode modeResize = GREEDY, 
									   int sizeBorder =nDefaultBorder, int sizeExtraBorder =0, 
									   int sizeSecondary =0);


	CPanePosition panePosition( layOrientation orientation, ETSLayoutMgr::layResizeMode modeResize = GREEDY, 
									   int sizeBorder =nDefaultBorder, int sizeExtraBorder =0, 
									   int sizeSecondary =0);

	class PaneSplitter : public Pane
	{
	public:   
		PaneSplitter(ETSSplitter * pSpl, ETSLayoutMgr * pMgr, layOrientation orientation);

		virtual bool	resizeTo(CRect& rcNewArea,bool bRedraw = false, bool bReSizeAlways = false);
		virtual bool	addPane( CPaneBase pItem );
		virtual bool	addPane( CPane pSubpane, layResizeMode modeResize, int sizeSecondary /* = 0 */);

		int		getOffsetConstrain(int offset);
		int		getCloseConstrain(int offset);
		int		getOpenConstrain(int offset);
		int		getMinConstrainHorz();
		int		getMinConstrainVert();
		void	setOpenConstrain();
		void	showPane(bool bShow);
		bool	resizeTo(int offset,bool bRedraw = false, bool bReSizeAlways = false);

		void	SetSplPosition(int pos);
		int		GetSplPosition();
	
		int		getSecondSize();
		int		IsClosed();
		bool	ForceResizeOuterPane(int offset, bool bRedraw);
		int		GetOuterPaneMinPosition();
	...
	private:
		...
		void ForceResizeTo(CPaneBase &pPane, CRect &rc,bool bRedraw = false);
	};
	class ETSGUI_EXT_CLASS  PaneMulti : public Pane
	{
	public:	
		PaneMulti(ETSLayoutMgr* pMgr, layOrientation orientation, int sizeBorder = nDefaultBorder, int sizeExtraBorder = 0 );
		
		virtual void	addLayer(CString name , CPaneBase pane);
		//virtual bool	addPane( CPane pSubpane, layResizeMode modeResize, int sizeSecondary /* = 0 */);		
	protected:
		CString		m_strCurrent;
		CPaneBase	m_paneCurrent;

		CMap<CString, LPCSTR ,ETSLayoutMgr::CPaneBase, ETSLayoutMgr::CPaneBase>  m_mapLayers;
	...
	public:
		...
		virtual bool	resizeTo(CRect& rcNewArea,bool bRedraw = false, bool bReSizeAlways = false);
		virtual void	showPane(bool bShow);
		virtual CPaneBase GetSubPane(CString name);

	public:
		CString		getCurrent();
		bool		isLayerExist(CString name);
		bool		setCurrent(CString name , bool bRedraw = true);

	};

	class ETSGUI_EXT_CLASS  PanePosition : public Pane
	{
	public:	
		PanePosition(ETSLayoutMgr* pMgr, layOrientation orientation, int sizeBorder = nDefaultBorder, int sizeExtraBorder = 0 );
		virtual bool resizeTo(CRect& rcNewArea,bool bRedraw = false, bool bReSizeAlways = false);
	};

	ETSLayoutMgr(CWnd* pWnd)	{ m_pWnd = pWnd; m_sizeRootBorders = CSize(0,0); m_nLCX = m_nRCX = m_nTCY = m_nBCY = 0; m_pSkin = NULL; m_clrBackground = 0; m_clrOriBackground = 0;m_pDC = NULL;};
	...
	void	setRootBorders(int lCx, int tCy, int rCx, int bCy) {m_nLCX = lCx; m_nRCX = rCx; m_nTCY = tCy; m_nBCY = bCy;};
	void	setBackgroundColor(COLORREF color){m_clrBackground = color;}
	void	setOriBackGroundColor(COLORREF oriColor){m_clrOriBackground = oriColor;};
	COLORREF getOriBackGroundColor(){return m_clrOriBackground;};
	...
	virtual void UpdateBkLayout(CDC *dc, BOOL bUseMemDC);
	/**
	...
	//CPane paneSpl(ETSSplitter * pSpl,  layOrientation orientation);
	/**
	...
	CPane CreateRoot(layOrientation orientation, int sizeBorder = nDefaultBorder, int sizeExtraBorder = 0 )
	{
		...
		if(orientation == NO_ORI)
			m_RootPane = new PanePosition(this, orientation);
		else
...
protected:
	 ...
	 *
	 **/ 
	int m_nLCX, m_nRCX, m_nTCY, m_nBCY;
 ...
 * Base class for the Layout function. Derive your own class
 * from this or derive it from CDialog and modify _all_
 * references to CFormView to ETSLayoutFrameWnd
 */
class ETSGUI_EXT_CLASS ETSLayoutFrameWnd : public ETSLayout::CBaseFrameWnd, public ETSLayoutMgr
{
// Construction
	DECLARE_DYNAMIC(ETSLayoutFrameWnd)
public:
	ETSLayoutFrameWnd(LPCTSTR strName = NULL);   // standard constructor
	virtual ~ETSLayoutFrameWnd();
	BOOL   m_bFirstTimeUse;

	//第一次跳转
	BOOL	m_bFirstJumpToNowPlaying;
	BOOL	m_bFirstJumpToDownload;
//	virtual void UpdateLayout();
