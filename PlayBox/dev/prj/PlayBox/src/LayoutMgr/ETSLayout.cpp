...
void ETSLayoutMgr::Layout(CRect& rcClient)
{
	if(rcClient.Height() && rcClient.Width()  && m_RootPane.IsValid())	
	{
		...
		if(m_pDC != NULL && m_strWndName == "toppanel")
			CIrregularBorderMgr::GetInstance()->DrawToppanelPng(m_pDC, rcClient);

	}
}

void	ETSLayoutMgr::PaneBase::setPosition(CString start_x, CString start_y , CString end_x, CString end_y, CString percent_start, CString percent_end, layPositionMode mode)
{
	m_start_x = start_x; 
	m_start_y = start_y; 
	m_end_x = end_x; 
	m_end_y = end_y; 
	m_percent_start = percent_start;
	m_percent_end = percent_end;
	m_posMode = mode;
};
...
{
	Pane* pPane = NULL;
	if(orientation == NO_ORI)
	{
		pPane = new PanePosition(this, orientation);
	}
	else
	{
		pPane = new Pane ( this, orientation, sizeBorder, sizeExtraBorder );
	}
...
void ETSLayoutMgr::UpdateLayout()
{
	...
	if( m_pWnd->IsKindOf( RUNTIME_CLASS( CDialog ) ) && !(m_pWnd->GetStyle()&WS_CHILD) ) {
		...
		// Min and Max info
		int minWidth	= m_RootPane->getMinConstrainHorz() + rcBorder.Width()  + 2*m_sizeRootBorders.cx + m_nLCX + m_nRCX;
		int minHeight	= m_RootPane->getMinConstrainVert() + rcBorder.Height() + 2*m_sizeRootBorders.cy + m_nTCY + m_nBCY;
		int maxWidth	= m_RootPane->getMaxConstrainHorz();
		if(maxWidth != -1) {
			maxWidth += rcBorder.Width()  + 2*m_sizeRootBorders.cx  + m_nLCX + m_nRCX;
			maxWidth = max(maxWidth, minWidth);
		}
		int maxHeight	= m_RootPane->getMaxConstrainVert();
		if(maxHeight != -1) {
			maxHeight += rcBorder.Height() + 2*m_sizeRootBorders.cy + m_nTCY + m_nBCY;
		...
		m_pWnd->MoveWindow(rcWindow, FALSE);
	}
	// Do the Layout
	rcClient = GetRect();
	...
	//Offset the lefttop corner
	rcClient.top	+= m_nTCY;
	rcClient.bottom -= m_nBCY;
	rcClient.left	+= m_nLCX;
	rcClient.right  -= m_nRCX;
								 ...
								 , int sizeXMin/*=-1*/, int sizeYMin/*=-1*/ ) : PaneBase( pMgr )
{
	...
	if(pWnd != NULL)
		::SetWindowPos(pWnd->m_hWnd, NULL, 0,0, m_sizeX, m_sizeY, SWP_NOMOVE);
}
...
void ETSLayoutMgr::PaneItem::showPane(bool bShow) 
{

	if(!IsWindow(this->m_hwndCtrl))
		return;

	this->m_bShow = bShow;

	bool bShouldShow = false;
	PaneBase* paneBase = this->m_paneParent;
	if(paneBase != NULL && bShow == true)
	{
		while(paneBase->m_bShow)
		{
			paneBase = paneBase->m_paneParent;
			if(paneBase == NULL)
			{
				bShouldShow = true;
				break;
	...
	if(bShow)
		::ShowWindow(m_hwndCtrl, SW_SHOW);
	else
		::ShowWindow(m_hwndCtrl, SW_HIDE);
}
...
int ETSLayoutMgr::PaneItem::getConstrainHorz(int sizeParent) 
{
	...
	//当RELATIVE_HORZ时， m_sizeX代表百分比
	if(this->m_rcRect.Width() >= 0 && !(m_modeResize & RELATIVE_HORZ))
		m_sizeX = m_rcRect.Width();
...
int ETSLayoutMgr::PaneItem::getConstrainVert(int sizeParent) 
{
	...
	//当RELATIVE_VERT时， m_sizeY代表百分比
	if(this->m_rcRect.Height() >= 0 && !(m_modeResize & RELATIVE_VERT))
		m_sizeY = m_rcRect.Height();
...
//void  ETSLayoutMgr::PaneItem::adjustSize() 
//{
//	if(m_hwndCtrl) {
//		CRect rcWnd;
//		::GetWindowRect( m_hwndCtrl, rcWnd );
//		this->m_sizeX = rcWnd.Width();
//		this->m_sizeY = rcWnd.Height();
//		this->m_rcSize.cx = m_sizeX;
//		this->m_rcSize.cy = m_sizeY;
//	}
//}
bool ETSLayoutMgr::PaneItem::resizeTo(CRect& rcNewArea,bool bRedraw,bool bReSizeAlways) 
{
	if(m_hwndCtrl) 
	{
		if(!bReSizeAlways && m_rcRect.EqualRect(&rcNewArea))
		{
			return true;
		}
		m_rcRect = rcNewArea;	
		...
		pTempWnd->MoveWindow( rcNewArea.left, rcNewArea.top, rcNewArea.Width(), rcNewArea.Height(), false );
		...
		if(bRedraw)
			::RedrawWindow(m_hwndCtrl,0,0, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE | RDW_ALLCHILDREN);
	//	::ShowWindow(this->m_hwndCtrl, SW_SHOW);
		this->showPane(true);
	}
	if(m_itemType == ETSLayoutMgr::TYPE_BMP)
	{
		m_rcRect = rcNewArea;	
		if(m_pMgr->m_pSkin == NULL)
			return false;

		CDibBitmap* pDib = m_pMgr->m_pSkin->GetDibBmp(this->GetName(), false);

		if(pDib != NULL)
		{
			pDib->SetCDibRect(rcNewArea);

			if(m_pMgr->m_pDC != NULL && m_pMgr->m_pDC->m_hDC != NULL)
			{
				CDib::DrawMode mode =CDib::STRETCH ;

				if(m_posMode & ETSLayoutMgr::DRAW_NO_STRECTH)
					mode = CDib::NORMAL;
				if(m_posMode & ETSLayoutMgr::DRAW_TILE)
					mode = CDib::TILE;


				if(m_posMode & ETSLayoutMgr::DRAW_CENTER_STRECH)
				{
					pDib->DrawStrechFromCenter(m_pMgr->m_pDC);
				}
				else if(m_posMode & ETSLayoutMgr::DRAW_TRANSPRANT)
				{
					pDib->Draw(m_pMgr->m_pDC, true, RGB(0,0,0), mode);
				}
				else if(m_posMode & ETSLayoutMgr::DRAW_TILE)
				{
					pDib->Draw(m_pMgr->m_pDC, true, RGB(0,0,0), mode);
				}
				else
				{
					pDib->Draw(m_pMgr->m_pDC, false, RGB(0,0,0), mode);
				}
			}
		}
...
bool ETSLayoutMgr::PaneTab::resizeTo(CRect& rcNewArea,bool bRedraw,bool bReSizeAlways)
{
	this->m_rcRect = rcNewArea;
	
	m_pTab->MoveWindow(rcNewArea);
	m_pTab->AdjustRect(FALSE,rcNewArea);
//	m_rcSize.cx = rcNewArea.Width();
//	m_rcSize.cy = rcNewArea.Height();
	return Pane::resizeTo(rcNewArea,bRedraw, bReSizeAlways);
}
...
bool ETSLayoutMgr::PaneCtrl::resizeTo(CRect& rcNewArea,bool bRedraw,bool bReSizeAlways)
{
  // FIX: ::MoveWindow would case problems with some ActiveX Controls [Micheal Chapman]
  m_rcRect = rcNewArea;
  CWnd* pTempWnd = CWnd::FromHandle( m_hwndCtrl );
...
ETSLayoutMgr::CPaneBase ETSLayoutMgr::Pane::GetSubPane(CString name)
{
	for(int i = 0 ; i<m_paneItems.GetSize(); i++)
	{
		...
		if(pItem->GetName() == name)
			...
			CPaneBase pSubItem = pItem->GetSubPane(name);
			if(pSubItem.IsValid())
				return pSubItem;
		}

	}
	return CPaneBase(NULL);
}
...
//		i = m_paneItems.GetSize();
//#pragma warning( default: 4244)	
	//pane不能嵌套相同ori的子pane，所以加入wrap一个ori相反的子pane。
	if( pSubpane->getOrientation() == m_Orientation)
	{
		...
		pPaneWrap->SetName(pSubpane->GetName() + "_wrap");
	...
	pSubpane.GetPaneBase()->m_paneParent = this;
	CString name = pSubpane.GetPaneBase()->GetName();
	return true;
}
...
#pragma warning( default: 4244)	
	...
	CPaneBase  Pane = m_paneItems[m_paneItems.GetSize() -1];

	pItem.GetPaneBase()->m_paneParent = this;
	CString name = pItem.GetPaneBase()->GetName();
	return true;
}

void ETSLayoutMgr::Pane::showPane(bool bShow) 
{
	m_bShow = bShow;
	for(int i=0; i<m_paneItems.GetSize(); ++i) {
		CPaneBase pItem = m_paneItems[i];
		pItem->showPane(bShow);
	}
}

int ETSLayoutMgr::Pane::getConstrainHorz(int sizeParent) 
{
	ASSERT( m_Orientation == VERTICAL || m_Orientation == NO_ORI);
	
	if(this->m_bClosed && m_Orientation == HORIZONTAL)
		return 0;

	if( m_modeResize & RELATIVE_HORZ ) {
		return (sizeParent * m_sizeSecondary) / 100;
	}
	else if(this->m_rcRect.Width() >= 0){
		return this->m_rcRect.Width();
	}
...
{
	if(this->m_bClosed && m_Orientation == VERTICAL || m_Orientation == NO_ORI)
		return 0;
	...
	else if(this->m_rcRect.Height() >= 0){
		return this->m_rcRect.Height();
	}
...
int ETSLayoutMgr::Pane::getMaxConstrainHorz() 
{
	if(m_Orientation == HORIZONTAL) {
		...
		return (nMaxConstr == -1) ? -1 : nMaxConstr + ((int)m_paneItems.GetUpperBound()*m_sizeBorder) + 2*m_sizeExtraBorder;
	}
...
{
	if(this->GetName().Find("_wrap") != -1)
	{
		return m_paneItems[0]->getMinConstrainHorz();
	}
	//????
	////if(m_Orientation == VERTICAL && m_sizeSecondary == 0)
	////	return 0;
	...
	if(m_Orientation == HORIZONTAL) {
		...
		return nMaxConstr + ((int)m_paneItems.GetUpperBound()*m_sizeBorder) + 2*m_sizeExtraBorder;
	}
	else {
		int nMaxConstr = 0;
		for(int i=0; i<m_paneItems.GetSize(); ++i) {
...
int ETSLayoutMgr::Pane::getMinConstrainVert() 
{
	if(this->GetName().Find("_wrap") != -1)
	{
		return m_paneItems[0]->getMinConstrainVert();
	}

	//if(m_Orientation == HORIZONTAL && m_sizeSecondary == 0)
	//	return 0;
										 ...
										 CArray<int,int>& sizeMin, CArray<int,int>& sizeMax)
{
	...
	for(int i=0; i<m_paneItems.GetSize(); ++i) {
		...
		if( m_Orientation == HORIZONTAL ) {
			...
			int test = pItem->getMaxConstrainVert();
									   ...
									   CArray<int,int>& sizeMin, CArray<int,int>& sizeMax)
{
	...
	//计算greedy pane的比例
	//if(m_aryGreedyProp.GetSize() != nGreedy)
	{
		float totalGreedy = 0;
		CArray<float, float> aryItemsSize;
		...
		for(int i=0; i<m_paneItems.GetSize(); ++i) {
			...
			)
			{
				if(m_Orientation==HORIZONTAL)
				{
					totalGreedy += pItem->getPaneRect().Width();
					aryItemsSize.Add((float)pItem->getPaneRect().Width());
				}
				else
				{
					totalGreedy += pItem->getPaneRect().Height();
					aryItemsSize.Add((float)pItem->getPaneRect().Height());
				}
			}
		}
			
		m_aryGreedyProp.RemoveAll();
		for(int i = 0 ; i<aryItemsSize.GetSize(); i++)
		{
			float d = aryItemsSize.GetAt(i);
			if(totalGreedy > 0)
				m_aryGreedyProp.Add(aryItemsSize.GetAt(i)/totalGreedy);
			else
				m_aryGreedyProp.Add(0);
		}
	}

	int nTotalGreedyCount = nGreedy;
	for(int i=0; i<m_paneItems.GetSize(); ++i) {
		CPaneBase pItem = m_paneItems[i];
		...
		)
		{
			...
			float prop = m_aryGreedyProp.GetAt(nTotalGreedyCount - nGreedy);
			int nSize = (int)((float)availSpace * prop);

			//if(m_Orientation==HORIZONTAL)
			//	nSize		= availSpace * pItem->getPaneRect() / totalGreedy;
			//else
			//	 nSize		= availSpace * pItem->getPaneSize().cy / totalGreedy;
...
bool ETSLayoutMgr::Pane::resizeTo(CRect& rcNewArea,bool bRedraw,bool bReSizeAlways) 
{
	if(!bReSizeAlways && m_rcRect.EqualRect(&rcNewArea))
	{
		return true;
	...
	int availSpace = (m_Orientation == HORIZONTAL ? rcNewArea.Width() : rcNewArea.Height() ) - ((int)m_paneItems.GetUpperBound()*m_sizeBorder);
	...
	for(int i=0; i<m_paneItems.GetSize(); ++i) {
		...
		if( m_Orientation==HORIZONTAL ) {
			...
			if( (pItem->m_modeResize & ALIGN_HCENTER) == ALIGN_HCENTER ) {
				rcPane.OffsetRect( (rcNewArea.Width() - rcPane.Width())/2, 0 ); 
				if(rcPane.left < 0)
					rcPane.OffsetRect(0-rcPane.left, 0);
			}
			else if( m_modeResize & ALIGN_RIGHT ) {
				rcPane.OffsetRect( rcNewArea.Width() - rcPane.Width(), 0 ); 
			}

			if(rcPane.left > this->m_rcRect.left)
				m_rcRect.left = rcPane.left;
		}
		else {
			rcPane.SetRect(rcNewArea.left, nOffset, rcNewArea.right, nOffset+sizePrimary[i]);

			if( (pItem->m_modeResize & ALIGN_VCENTER) == ALIGN_VCENTER ) {
				rcPane.OffsetRect( 0, (rcNewArea.Height()-rcPane.Height())/2 ); 
			}
			else if( m_modeResize & ALIGN_BOTTOM ) {
				rcPane.OffsetRect( 0, rcNewArea.Height() - rcPane.Height()); 
			}

			if(rcPane.bottom > m_rcRect.bottom)
				m_rcRect.bottom = rcPane.bottom;
		}

		// do the resizing!
		pItem->resizeTo( rcPane , bRedraw, bReSizeAlways);
