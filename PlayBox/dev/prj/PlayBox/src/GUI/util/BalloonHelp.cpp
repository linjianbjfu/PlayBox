...
#define nCXCloseBtn 14
#define nCYCloseBtn 14
               ...
               unsigned int unTimeout /*= 10000*/)
{
   ...
   //   CSize sizeIcon(max(::GetSystemMetrics(SM_CXSIZE), ::GetSystemMetrics(SM_CXSMICON)), max(::GetSystemMetrics(SM_CYSIZE), ::GetSystemMetrics(SM_CYSMICON)));
	  ...
	  //HBITMAP hIcon = LoadBitmap(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDB_BMPDrag));
   //   if (NULL != hIcon)
		 // pbh->SetIcon(hIcon, RGB(255,255,255)); // bitmap, transparent color
	  //DeleteObject( hIcon );
   //}
...
void CBalloonHelp::SetTimeout(unsigned int unTimeout)
{
   ...
   if ( NULL != m_hWnd )
   {
      if ( m_unTimeout > 0 )
      {
         m_unTimerClose = (unsigned int) SetTimer(ID_TIMER_CLOSE, m_unTimeout, NULL);
      }
      else
      {
...
CSize CBalloonHelp::DrawHeader(CDC* pDC, bool bDraw)
{
   ...
   {
      int nBtnWidth = nCXCloseBtn;//::GetSystemMetrics(SM_CXSIZE);
      ...
      if (bDraw)
         pDC->DrawFrameControl(CRect(rectClient.right-nBtnWidth,0,rectClient.right,nCYCloseBtn/*::GetSystemMetrics(SM_CYSIZE)*/), DFC_CAPTION, DFCS_CAPTIONCLOSE|DFCS_FLAT);
...
void CBalloonHelp::PositionWindow()
{
   ...
   regionRound.CreateRectRgn(ptTopLeft.x,ptTopLeft.y,ptBottomRight.x,ptBottomRight.y);
...
void CBalloonHelp::OnLButtonDown(UINT, CPoint point) 
{
   if (m_unOptions & unSHOW_CLOSE_BUTTON)
   {
      CRect rect;
      GetClientRect(&rect);
      rect.left = rect.right - nCXCloseBtn;//::GetSystemMetrics(SM_CXSIZE);
      rect.bottom = rect.top + nCYCloseBtn;//::GetSystemMetrics(SM_CYSIZE);
