...
// Robert Edward Caldecott,Kenny Goers,Leonardo Zide,ext
...
{
	int len=MultiByteToWideChar(CP_ACP, 0,szAnsiString, -1,NULL,0);
	wchar_t* wszUtf8 = new wchar_t[len];
	memset(wszUtf8, 0, len * 2);
	MultiByteToWideChar(CP_ACP, 0, szAnsiString, -1, (LPWSTR)wszUtf8, len);	

	SetWideString( wszUtf8);  //SK:  see MFC Tech Note 059
	delete[] wszUtf8;
}
...
//depending on the MFC-Version we are using
{
	...
	if (m_szMenuText)
    {
		...
		WideCharToMultiByte( CP_ACP, 0, m_szMenuText, -1,
			szbuffer, 256, NULL, NULL );
...
//added by vito--
	if(IsWinXPLuna())m_menuItemBitmapBkColor=GetSysColor(COLOR_3DFACE);
	else m_menuItemBitmapBkColor=GetSysColor(COLOR_MENU);

	m_menuItemBkColor=GetSysColor(COLOR_WINDOW);
	m_menuItemTextColor= GetSysColor(COLOR_MENUTEXT);
	m_menuItemSelectBKColor = GetSysColor(COLOR_HIGHLIGHT);
//==
}
...
*/
	COLORREF m_newclrBack=m_menuItemBitmapBkColor;
	COLORREF m_clrBack=m_menuItemBkColor;
//--
	...
	if(state & MF_SEPARATOR){
		...
		BOOL checkflag=FALSE;
		COLORREF crText ;
		crText = m_menuItemTextColor;
		
		
		COLORREF crSelect = m_menuItemSelectBKColor;
...
BOOL BCMenu::AppendODMenuA(LPCSTR lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	int len=MultiByteToWideChar(CP_ACP, 0,lpstrText, -1, NULL,0);
	wchar_t* wszUtf8 = new wchar_t[len];
	memset(wszUtf8, 0, len * 2);
	MultiByteToWideChar(CP_ACP, 0, lpstrText, -1, (LPWSTR)wszUtf8, len);	

	BOOL bRet = AppendODMenuW( wszUtf8,nFlags,nID,nIconNormal);//SK: See MFC Tech Note 059
                           ...
                           CImageList *il,int xoffset)
{
	...
	BOOL bRet = AppendODMenuW( wszUtf8,nFlags,nID,il,xoffset);
	delete[] wszUtf8;
                           ...
                           int nIconNormal)
{
	...
	BOOL bRet = InsertODMenuW(nPosition,wszUtf8,nFlags,nID,nIconNormal);
	delete[] wszUtf8;
                           ...
                           CImageList *il,int xoffset)
{
	...
	BOOL bRet = InsertODMenuW(nPosition,wszUtf8,nFlags,nID,il,xoffset);
	delete[] wszUtf8;
...
BOOL BCMenu::ModifyODMenuA(const char * lpstrText,UINT nID,int nIconNormal)
{
	...
	BOOL bRet = ModifyODMenuW( wszUtf8,nID,nIconNormal);//SK: see MFC Tech Note 059
	delete[] wszUtf8;
	return bRet;
}
...
//added by vito--
	psubmenu->SetMenuItemBkColor( m_menuItemBkColor );
	psubmenu->SetMenuItemBitmapBkColor( m_menuItemBitmapBkColor );
//--
...
BOOL BCMenu::ModifyODMenuA(const char * lpstrText,UINT nID,CImageList *il,int xoffset)
{
	...
	BOOL bRet = ModifyODMenuW( wszUtf8,nID,il,xoffset);
...
{	
	...
	BOOL bRet = ModifyODMenuW( wszUtf8,nID,bmp);
...
BOOL BCMenu::ModifyODMenuA(const char * lpstrText,UINT nID,COLORREF fill,COLORREF border,int hatchstyle,CSize *pSize)
{
	...
	BOOL bRet = ModifyODMenuW( wszUtf8,nID,fill,border,hatchstyle,pSize);
	delete[] wszUtf8;
                           ...
                           int nIconNormal)
{
	...
	len=MultiByteToWideChar(CP_ACP, 0,OptionText, -1, NULL,0);
	wchar_t* wszUtf82 = new wchar_t[len];
	memset(wszUtf82, 0, len * 2);
	MultiByteToWideChar(CP_ACP, 0, OptionText, -1, (LPWSTR)wszUtf82, len);	

	BOOL bRet = ModifyODMenuW( wszUtf8,wszUtf82,nIconNormal);//SK: see MFC  Tech Note 059
	delete[] wszUtf8;
...
void LineTo(CDC *pDC, CPoint pFrom, CPoint pTo, COLORREF clr)
{
	CPoint * p1;
	CPoint * p2;
	...
	for(int i = p1->x; i<=p2->x; i++)
	{
		...
		if(p2->x - i != 0)
			deltaY = ((float)(p2->y - p1->y ))  * ((float)(p2->x - p1->x))
			/(float)(p2->x - i);
		else
			deltaY = 0;
		
		pDC->SetPixel(i, p1->y, clr);
		pDC->SetPixel(i, p1->y + 1, clr);
	}
}
...
void BCMenu::DrawCheckMark(CDC* pDC,int x,int y,COLORREF color,BOOL narrowflag)
{
	...
	m_penBack.CreatePen(PS_SOLID,1,color);
	CPen *pOldPen = pDC->SelectObject (&m_penBack);
	if(narrowflag)dp=1;

	LineTo(pDC, CPoint(x,y+2), CPoint(x,y+5-dp), color);
	LineTo(pDC, CPoint(x+1,y+3), CPoint(x+1,y+6-dp), color);
	LineTo(pDC, CPoint(x+2,y+4), CPoint(x+2,y+7-dp), color);
	LineTo(pDC, CPoint(x+3,y+3), CPoint(x+3,y+6-dp), color);
	LineTo(pDC, CPoint(x+4,y+2), CPoint(x+4,y+5-dp), color);
	LineTo(pDC, CPoint(x+5,y+1), CPoint(x+5,y+4-dp), color);
	LineTo(pDC, CPoint(x+6,y), CPoint(x+6,y+3-dp), color);
...
BOOL BCMenu::Draw3DCheckmark(CDC *dc1, const CRect& rc,
...
{
	int len=MultiByteToWideChar(CP_ACP, 0,lpszNewItem, -1, NULL,0);
	wchar_t* wszUtf8 = new wchar_t[len];
	memset(wszUtf8, 0, len * 2);
	MultiByteToWideChar(CP_ACP, 0, lpszNewItem, -1, (LPWSTR)wszUtf8, len);	

	BOOL bRet = AppendMenuW(nFlags,nIDNewItem,wszUtf8,nIconNormal);
	delete[] wszUtf8;
...
BOOL BCMenu::AppendMenuA(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CImageList *il,int xoffset)
{
	...
	BOOL bRet = AppendMenuW(nFlags,nIDNewItem,wszUtf8,il,xoffset);
	delete[] wszUtf8;
...
BOOL BCMenu::AppendMenuA(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CBitmap *bmp)
{
	...
	BOOL bRet = AppendMenuW(nFlags,nIDNewItem,wszUtf8,bmp);
	delete[] wszUtf8;
	return bRet;
}
...
{
	wchar_t* wszUtf8 = NULL;
	if(lpszNewItem != NULL)
	{
		int len=MultiByteToWideChar(CP_ACP, 0,lpszNewItem, -1, NULL,0);
		wszUtf8 = new wchar_t[len];
		memset(wszUtf8, 0, len * 2);
		MultiByteToWideChar(CP_ACP, 0, lpszNewItem, -1, (LPWSTR)wszUtf8, len);	
	}

	BOOL bRet = InsertMenuW(nPosition,nFlags,nIDNewItem,wszUtf8,nIconNormal);
	delete[] wszUtf8;
...
{	
	...
	BOOL bRet = InsertMenuW(nPosition,nFlags,nIDNewItem,wszUtf8,il,xoffset);
	delete[] wszUtf8;
...
BOOL BCMenu::InsertMenuA(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CBitmap *bmp)
{
	...
	BOOL bRet = InsertMenuW(nPosition,nFlags,nIDNewItem,wszUtf8,bmp);
	delete[] wszUtf8;
...
BCMenu* BCMenu::AppendODPopupMenuA(LPCSTR lpstrText)
{
	...
	BCMenu* bRet = AppendODPopupMenuW( wszUtf8 );
	delete[] wszUtf8;
...
BCMenu* BCMenu::GetSubBCMenu(char* pText)
{
	...
	wchar_t* wszUtf8 = new wchar_t[len];
	memset(wszUtf8, 0, len * 2);
	MultiByteToWideChar(CP_ACP, 0, pText, -1, (LPWSTR)wszUtf8, len);	
	
	BCMenu* pMenu = GetSubBCMenu( wszUtf8);
	delete[] wszUtf8;
...
int BCMenu::GetMenuPosition(char* pText)
{
	...
	int iRet = GetMenuPosition( wszUtf8);
	delete[] wszUtf8;
...
int BCMenu::RemoveMenu(char* pText, BC_Seperator sPos)
{
	...
	int iRet = RemoveMenu( wszUtf8, sPos);
	delete[] wszUtf8;
...
{	
	...
	int iRet = DeleteMenu( wszUtf8, sPos);
	delete[] wszUtf8;
...
//added by vito
void BCMenu::SetMenuItemBkColor( COLORREF col )
{ 
	m_menuItemBkColor = col;	
}

void BCMenu::SetMenuItemBitmapBkColor( COLORREF col )
{ 
	m_menuItemBitmapBkColor = col; 	
	m_bitmapBackground      = col;
}

void BCMenu::SetTextColor( COLORREF col )
{
	m_menuItemTextColor = col;
}

void BCMenu::SetMenuItemBitmap(UINT nID, LPCTSTR text, HBITMAP hBitmap )
{
	CBitmap bmp;
	ASSERT(hBitmap);
	if (bmp.m_hObject)
	...
	bmp.Attach(hBitmap);
	ModifyODMenuA( text, nID, &bmp );
}

void BCMenu::SetMenuItemBitmap( UINT nID, LPCTSTR text, LPCSTR bitmap )
{
	CBitmap bmp;
	HBITMAP hBitmap = NULL;
	hBitmap = (HBITMAP)::LoadImage(NULL, bitmap, IMAGE_BITMAP, 
