...
CHyperLink::CHyperLink()
{
    ...
    m_strURL.Empty();
    m_nTimerID          = 100;
	m_StdFont.m_hObject           = NULL;
	m_UnderlineFont.m_hObject    = NULL;
}
...
void CHyperLink::PreSubclassWindow() 
{
	...
	if( m_StdFont.m_hObject == NULL )
	{
		m_StdFont.CreateFontIndirect(&lf);
	}

    lf.lfUnderline = (BYTE) TRUE;
	if( m_UnderlineFont.m_hObject == NULL )
	{
...
void CHyperLink::SetToolTip(CString str)
{
	CRect rect; 
	GetClientRect(rect);
	m_ToolTip.AddTool(this, str, rect, TOOLTIP_ID);
}
