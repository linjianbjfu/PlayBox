...
public:
	void SetTransMask(BOOL bMaskShow){m_bHasMask = bMaskShow;};
	bool	m_bPause;
	BOOL GetPaintRect(RECT *lpRect);
	BOOL SetPaintRect(const RECT *lpRect);
	CPictureEx(bool bStretch = false);
	virtual ~CPictureEx();
	...
	void InitAnimation(HWND hwnd){m_hWnd = hwnd;};
...
//	afx_msg void OnDestroy();
	...
	CArray<CRect, CRect> *m_aryPoint;
	CArray<CRect, CRect> *m_aryPrepPoint;
...
public:
	...
	void	AddPrepDrawPoint(CRect rc)
	{
			m_aryPrepPoint->Add(rc);
	}

	void   ClearPrepDrawPoint()
	{	
		m_aryPrepPoint->RemoveAll();
	}

	bool	IsDrawPointChanged()
	{
		if(m_aryPrepPoint->GetSize() != m_aryPoint->GetSize())
			return true;
		for(int i = 0 ; i<m_aryPrepPoint->GetSize(); i++)
		{
			CRect rc1, rc2;

			rc1 = m_aryPrepPoint->GetAt(i);
			rc2 = m_aryPoint->GetAt(i);
			if(!rc1.EqualRect(&rc2))
				return true;
		}
		return false;
	}

	bool    RefreshDrawPoint()
	{
		if(!IsDrawPointChanged())
			return true;

		m_aryPoint->RemoveAll();
		for(int i = 0 ; i<m_aryPrepPoint->GetSize(); i++)
		{
			m_aryPoint->Add(m_aryPrepPoint->GetAt(i));
		}
	//	InitAnimation(m_hWnd);
		Draw();
		m_aryPrepPoint->RemoveAll();
		return true;
	}

	void OnPaint(HDC dc = NULL);
	
//	CCriticalSection _csGifOnPaint;
};
