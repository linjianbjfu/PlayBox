...
CPictureEx::CPictureEx( bool bStretch)
: m_bStretch(bStretch)
{
	...
	m_aryPoint = new CArray<CRect, CRect>;
	m_aryPrepPoint = new CArray<CRect , CRect>;
...
BOOL CPictureEx::HitTest(CPoint p)
{
	for(int i = 0 ; i<m_aryPoint->GetSize(); i++)
	{
		
		CRect rc = m_aryPoint->GetAt(i);
		CPoint p1 = rc.CenterPoint();
		p1.Offset(-m_PictureSize.cx/2 , - m_PictureSize.cy/2);
		CRect r(p1.x, p1.y, p1.x + m_PictureSize.cx , p1.y + m_PictureSize.cy);
...
void CPictureEx::OnPaint(HDC memdc) 
{
	//CPaintDC dc(this); // device context for painting
	////Lock lock(_csGifOnPaint);;
	if(m_bPause)
		return;
	HDC dc = NULL;
	int iSaveDC = 0;
	...
	for(int i = 0 ; i<m_aryPoint->GetSize(); i++)
	{
		...
		p.Offset(-m_PictureSize.cx/2 , - m_PictureSize.cy/2);
		...
		if(!m_bStretch)
		{
			if(m_bHasMask)
			{
				StretchBlt(dc, p.x, p.y,  m_PictureSize.cx, m_PictureSize.cy,
					dc, rc.right, p.y, 1, m_PictureSize.cy, SRCCOPY);

				::TransparentBlt(dc, p.x, p.y, m_PictureSize.cx, m_PictureSize.cy,
					m_hMemDC, 0, 0, m_PictureSize.cx, m_PictureSize.cy,RGB(0,0,0));
			}
			else
			{
				BitBlt(dc, p.x, p.y, m_PictureSize.cx, m_PictureSize.cy, m_hMemDC, 0, 0,SRCCOPY);
			}
		}
		else
		{
			SetStretchBltMode(dc,STRETCH_DELETESCANS); 
			StretchBlt(dc, rc.left, rc.top, rc.Width(), rc.Height(), m_hMemDC, 0, 0,m_PictureSize.cx, m_PictureSize.cy, SRCCOPY);
		}		
	}

	if(memdc == NULL)
	{
		RestoreDC( dc, iSaveDC );
		::ReleaseDC(m_hWnd, dc);
	}
}
