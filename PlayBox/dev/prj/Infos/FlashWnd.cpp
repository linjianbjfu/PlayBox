...
BOOL CFlashWnd::OnBeforeShowingContent()
{
	...
	if ((m_lVersion&0x00FF0000)==0x00080000 || (m_lVersion&0x00FF0000)>=0x000a0000)
...
BOOL CFlashWnd::OnAfterShowingContent()
{
	...
	hr = m_lpControl->LoadMovie(0, m_strMovie.c_str());
	if (FAILED(hr))
		return FALSE;
	m_bStoped = FALSE;
	hr = m_lpControl->Play();
...
BOOL CFlashWnd::StopFlash()
{
	HRESULT hr = m_lpControl->Stop();
	m_bStoped = TRUE;
	if (FAILED(hr))
		return FALSE;
	return TRUE;
    ...
    /* [out] */ UINT __RPC_FAR *puArgErr)
{
	...
	{
	case 0x96:
		//::SendMessage(m_hWndParent, WM_DESTROY, 0, 0);
		::SendMessage(m_hWndParent, IDC_NEXT_FLASH, 0, 0);
		break;
	default:
		return S_OK;
	}
